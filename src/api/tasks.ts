...
export async function getTasksByProject(
  ...
  const response = await axios.get<TaskApiResponse>(
    `${BASE_URL}/tasks?projectId=${projectId}&status=${status}&priority=${priority}&sortOrder=${sortOrder}`,
    {
      headers: {
...
export async function getTasks(
  ...
  const response = await axios.get<TaskApiResponse>(
    `${BASE_URL}/tasks?status=${status}&priority=${priority}&sortOrder=${sortOrder}`,
    {
      headers: {
        Authorization: `Bearer ${token}`
      }
