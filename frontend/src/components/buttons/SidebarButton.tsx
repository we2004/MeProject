...
function SidebarButton({ Icon, text, path }: SidebarButtonProps) {
  return (
    <NavLink
      to={path}
      className={({ isActive }) =>
        `group flex lg:h-15 lg:w-17 h-12 w-12 flex-col items-center justify-center rounded-2xl text-center transition-all duration-300 ${
          isActive
    ...
    >
      ...
      <p className="text-sm mt-1 font-heading font-semibold hidden lg:block">{text}</p>
    </NavLink>
  )
}
