...
const allowedTypes = ['image/png', 'image/jpeg', 'image/jpg', 'image/svg+xml', 'application/pdf', 'text/markdown', 'text/plain'];
...
router.get('/:id', async (req: AuthRequest, res: Response) => {
  ...
  try {
    const attachment = await db.get('SELECT * FROM attachments WHERE id = ? AND userId = ?', [req.params.id, userId]);
...
router.post('/', upload.single('file'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const type = ['png', 'jpg', 'jpeg', 'svg', 'pdf', 'md', 'txt'].includes(ext) ? ext : 'txt';
    ...
    res.status(201).json({ id: result.lastID, message: 'Attachment uploaded' });
  } catch (error: any) {
    if (req.file) {
      fs.unlinkSync(req.file.path);
    }
...
router.delete('/:id', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await db.run('DELETE FROM attachments WHERE id = ? AND userId = ?', [req.params.id, userId]);
